...
class ApprovalsService {
  ...
  async listWorkflows(input: ListWorkflowsInput) {
    ...
    const [data, total] = await prisma.$transaction([
      prisma.approvalWorkflow.findMany({ where, include: workflowInclude, orderBy: { priority: 'desc' }, skip, take: limit }),
  ...
  async findWorkflowById(id: string) {
    const wf = await prisma.approvalWorkflow.findUnique({ where: { id }, include: workflowInclude })
  ...
  async updateWorkflow(id: string, input: UpdateWorkflowInput, _userId: string) {
    ...
    const hasPending = await prisma.approvalRequest.findFirst({ where: { workflowId: id, status: 'pending' } })
  ...
  async deleteWorkflow(id: string) {
    ...
    return prisma.approvalWorkflow.update({ where: { id }, data: { isActive: false } })
  }
  ...
  async requestApproval(params: {
    ...
    // Snapshot: keep only key fields, avoid bloating
  ...
  async approve(requestId: string, userId: string, input: ApproveInput) {
    ...
    if (isFinalStep) {
      ...
      const nextStep = await prisma.approvalWorkflowStep.findFirst({
  ...
  async delegate(requestId: string, userId: string, input: DelegateInput) {
    ...
    const delegatee = await prisma.user.findUnique({ where: { id: input.delegatedToId }, select: { id: true, firstName: true } })
    ...
    // Update step to point to delegated user
  ...
  async listRequests(input: ListRequestsInput, currentUserId: string) {
    ...
    // Load current user's role for isMyTurn computation
    ...
    // Apply pendingForMe filter after isMyTurn computation
  ...
  async findRequestById(id: string) {
    const request = await prisma.approvalRequest.findUnique({ where: { id }, include: requestInclude })
  ...
  async pendingCount(userId: string): Promise<number> {
    ...
    // Count requests where it's the user's turn
  ...
  private async _onApprovalCompleted(request: ApprovalRequest) {
    // Emit event for downstream handlers — each module lifts its own block
    // Expenses auto-transition to approved; others require manual action post-approval
  ...
  private async _onApprovalRejected(request: ApprovalRequest) {
    // Revert document to draft based on type
    switch (request.documentType) {
      case 'invoice':
        await prisma.invoice.update({ where: { id: request.documentId }, data: { status: 'draft' } }).catch(() => {})
        break
      case 'proforma':
        await prisma.proforma.update({ where: { id: request.documentId }, data: { status: 'draft' } }).catch(() => {})
        break
      case 'expense':
        await prisma.expense.update({ where: { id: request.documentId }, data: { status: 'draft' } }).catch(() => {})
        break
      case 'purchase_order':
        await prisma.purchaseOrder.update({ where: { id: request.documentId }, data: { status: 'draft' } }).catch(() => {})
        break
      case 'supplier_invoice':
        await prisma.supplierInvoice.update({ where: { id: request.documentId }, data: { status: 'draft' } }).catch(() => {})
        break
    }
  }
}
