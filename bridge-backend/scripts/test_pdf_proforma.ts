/**
 * Script de test : génération d'une proforma PDF.
 * Exécuter depuis bridge-backend/ :
 *   npx tsx test_pdf_proforma.ts
 */
...
const params = {
  ...
  number:             'BTS/DC/2026/03/PFM015',
  issueDate:          '10/03/2026',
  validUntil:         '10/04/2026',
  ...
  subject:            'Fourniture et installation infrastructure réseau et serveurs',
  lines,
  subtotalHt,
  totalTax,
  totalTtc,
  deliveryDelay:      '30 jours ouvrés à compter de la réception de l\'acompte',
  warranty:           '12 mois pièces et main d\'œuvre sur les équipements livrés',
  paymentConditions:  '30% à la commande, solde à la livraison — Virement bancaire — Afriland First Bank N° 10006-00010-00123456789-77',
  currency:           'XAF',
  notes:              'Cette proforma est valable 30 jours. Tout équipement commandé sera livré neuf, sous emballage d\'origine avec certificat de garantie constructeur.',
};

async function main() {
  console.log(`Génération Proforma — ${lines.length} lignes...`);
  ...
  const outPath = path.resolve('./test_proforma.pdf');
