/**
 * Script de test : génération d'une facture d'acompte PDF.
 * Exécuter depuis bridge-backend/ :
 *   npx tsx test_pdf_acompte.ts
 */
...
const acompteTtc        = acompteHt + acompteTax;

const params = {
  type:               'Facture Acompte' as const,
  number:             'BTS/DC/2026/03/ACP007',
  ...
  subtotalHt,
  totalTax,
  totalTtc:           acompteTtc,   // TTC de l'acompte uniquement
  acomptePercentage,
  acompteHt,
  acompteTax,
  currency:           'XAF',
  paymentConditions:  'Acompte de 30% à la commande — Virement bancaire — Afriland First Bank N° 10006-00010-00123456789-77',
  notes:              'Cet acompte correspond à 30% du montant total du projet. Le solde sera facturé à la livraison et réception des équipements.',
};

// ── Génération ────────────────────────────────────────────────────────────────

async function main() {
  console.log(`Génération Facture Acompte — ${lines.length} lignes...`);
  console.log(`Sous-total HT projet : ${subtotalHt.toLocaleString('fr-FR')} XAF`);
  console.log(`Acompte HT (30%)     : ${Math.round(acompteHt).toLocaleString('fr-FR')} XAF`);
  console.log(`TVA sur acompte      : ${Math.round(acompteTax).toLocaleString('fr-FR')} XAF`);
  console.log(`Total TTC acompte    : ${Math.round(acompteTtc).toLocaleString('fr-FR')} XAF`);

  const html   = buildDocumentHtml(params);
  const pdfBuf = await generatePdf(html);

  const outPath = path.resolve('./test_facture_acompte.pdf');
