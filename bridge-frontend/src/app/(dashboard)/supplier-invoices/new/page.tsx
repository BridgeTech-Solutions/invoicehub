...
import { useCreateSupplierInvoice } from '@/features/supplier-invoices/hooks'
...
export default function NewSupplierInvoicePage() {
  ...
  const { data: posData } = usePurchaseOrders({ supplierId: supplierId || undefined, status: 'received', limit: 100 })
  ...
  const totals = lines.reduce((acc, l) => {
    const c = calcLine(l)
    return { ht: acc.ht + c.ht, tax: acc.tax + c.tax, ttc: acc.ttc + c.ttc }
  }, { ht: 0, tax: 0, ttc: 0 })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 1080, animation: 'page-in 0.2s ease' }}>
      <div>
        ...
        <PageHeader title="Nouvelle facture fournisseur" description="Saisissez une facture reçue d'un fournisseur" />
      </div>
          ...
          <div className="card" style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
              ...
              <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: 0 }}>
                <label style={lbl}>Fournisseur <span style={{ color: '#dc2626' }}>*</span></label>
                <select value={supplierId} onChange={e => { setSupplierId(e.target.value); setPurchaseOrderId(''); setErrors(er => ({ ...er, supplierId: '' })) }}
              ...
              {supplierId && purchaseOrders.length > 0 && (
                <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: 0 }}>
                  <label style={lbl}>Bon de commande lié (facultatif)</label>
                  <select value={purchaseOrderId} onChange={e => setPurchaseOrderId(e.target.value)}
                    ...
                    {purchaseOrders.map(po => <option key={po.id} value={po.id}>{po.number} · {new Intl.NumberFormat('fr-FR').format(Math.round(po.totalTtc))} XAF</option>)}
                  </select>
                </div>
              )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                <label style={lbl}>Date de facture <span style={{ color: '#dc2626' }}>*</span></label>
                <input type="date" value={invoiceDate} onChange={e => setInvoiceDate(e.target.value)}
                  style={{ ...inp, borderColor: errors.invoiceDate ? '#dc2626' : 'var(--border)' }}
                  onFocus={e => (e.target.style.borderColor = 'var(--primary)')}
                  onBlur={e  => (e.target.style.borderColor = errors.invoiceDate ? '#dc2626' : 'var(--border)')} />
                {errors.invoiceDate && <span style={{ fontSize: 11.5, color: '#dc2626', marginTop: 4 }}>{errors.invoiceDate}</span>}
              </div>
            ...
            {lines.map((l, i) => {
              ...
              return (
                <div key={l._id} style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1fr 0.8fr 0.5fr 36px', gap: 8, alignItems: 'start', marginBottom: 8 }}>
                  ...
                  <input value={l.unit} onChange={e => setLine(l._id, 'unit', e.target.value)} placeholder="U" style={inp}
                    onFocus={e => (e.target.style.borderColor = 'var(--primary)')}
                    onBlur={e  => (e.target.style.borderColor = 'var(--border)')} />
