...
import { useSupplierInvoice, useUpdateSupplierInvoice } from '@/features/supplier-invoices/hooks'
import { useSuppliers } from '@/features/suppliers/hooks'
import { ROUTES, TAX_RATE_DEFAULT } from '@/lib/constants'
import { formatXAF } from '@/lib/utils'
import type { CreateSILine } from '@/features/supplier-invoices/types'

interface LineState extends CreateSILine { _id: string }
let _lid = 0
...
function calcLine(l: LineState) {
  ...
  const r = (l.taxRate ?? TAX_RATE_DEFAULT) / 100
  return { ht, tax: ht * r, ttc: ht * (1 + r) }
}
...
export default function EditSupplierInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const updateMutation = useUpdateSupplierInvoice(id)
  ...
  const [accountingAccount,  setAccountingAccount]  = useState('401000')
  ...
  useEffect(() => {
    if (inv && !ready) {
      ...
      setAccountingAccount(inv.accountingAccount ?? '401000')
      setNotes(inv.notes ?? '')
      setLines(inv.lines.map(l => ({
        _id: String(++_lid),
        designation: l.designation, description: l.description ?? '',
        unit: l.unit, quantity: l.quantity, unitPriceHt: l.unitPriceHt, taxRate: l.taxRate,
      })))
      setReady(true)
    }
  }, [inv, ready])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 1080, animation: 'page-in 0.2s ease' }}>
      <div>
        <Link href={`${ROUTES.SUPPLIER_INVOICES}/${id}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--text-3)', textDecoration: 'none', marginBottom: 12 }}
          onMouseEnter={e => (e.currentTarget.style.color = 'var(--primary)')}
          onMouseLeave={e => (e.currentTarget.style.color = 'var(--text-3)')}>
          <ChevronLeft size={14} /> {inv?.number ?? 'Facture'}
        </Link>
        <PageHeader title="Modifier la facture fournisseur" description={`Mise à jour de ${inv?.number ?? ''}`} />
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card" style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                ...
                <input value={supplierRef} onChange={e => setSupplierRef(e.target.value)} style={inp} onFocus={e => (e.target.style.borderColor = 'var(--primary)')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                <label style={lbl}>Date de facture *</label>
                <input type="date" value={invoiceDate} onChange={e => setInvoiceDate(e.target.value)}
                  style={{ ...inp, borderColor: errors.invoiceDate ? '#dc2626' : 'var(--border)' }} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                ...
                <input value={accountingAccount} onChange={e => setAccountingAccount(e.target.value)} style={inp} onFocus={e => (e.target.style.borderColor = 'var(--primary)')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                <label style={lbl}>Notes</label>
                <input value={notes} onChange={e => setNotes(e.target.value)} style={inp} onFocus={e => (e.target.style.borderColor = 'var(--primary)')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} />
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 20 }}>
            <div style={{ display: 'flex', gap: 12 }}>
              <Link href={`${ROUTES.SUPPLIER_INVOICES}/${id}`}
