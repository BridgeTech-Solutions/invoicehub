...
function RecurringDetailView({ id }: { id: string }) {
  ...
  const lineTotal = template.lines.reduce(
    (sum, l) => sum + Number(l.unitPriceHt) * Number(l.quantity), 0,
  )
  const taxTotal = template.lines.reduce(
    (sum, l) => sum + Number(l.unitPriceHt) * Number(l.quantity) * (Number(l.taxRate) / 100), 0,
  )
      ...
      <div className="card" style={{ padding: '20px 24px' }}>
        ...
        {canEdit && (
          <div style={{ marginTop: 18, paddingTop: 18, borderTop: '1px solid var(--border)', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {canDelete && (
              <button
                onClick={() => { if (confirm('Supprimer ce gabarit de facturation récurrente ?')) deleteM.mutate(template.id) }}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 520 }}>
                ...
                <tbody>
                  {template.lines.map((line, i) => {
                    const lineHt = Number(line.unitPriceHt) * Number(line.quantity)
                    return (
                      <tr key={line.id} style={{ borderBottom: '1px solid var(--border)' }}>
                        ...
                        <td style={{ padding: '11px 10px', textAlign: 'right', fontFamily: 'var(--font-mono)', fontSize: 13.5, fontWeight: 600, color: 'var(--text-1)', whiteSpace: 'nowrap' }}>
                          {new Intl.NumberFormat('fr-FR').format(Math.round(lineHt))} XAF
                        </td>
                      </tr>
                    )
                  })}
